                    ...
                    <div className='flex items-center gap-4'>
                                ...
                                <div className='flex items-center gap-2 cursor-pointer group relative'>
                                    ...
                                    <div className='absolute top-0 right-0 pt-14 text-base font-medium text-gray-600 z-20 hidden group-hover:block '>
                                        <div className='min-w-48 bg-stone-100 rounded flex flex-col gap-3 p-4'>
                                            ...
                                            <p onClick={logout} className='hover:text-gray-800 cursor-pointer'>Logout</p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all duration-500`}>
                            ...
                            <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-base font-medium'>
                                <NavLink className='w-full' onClick={()=>setShowMenu(false)} to="/"><p className='px-4 py-2'>HOME</p></NavLink>
                                <NavLink className='w-full' onClick={()=>setShowMenu(false)} to="/doctors"><p className='px-4 py-2'>ALL DOCTORS</p></NavLink>
                                <NavLink className='w-full' onClick={()=>setShowMenu(false)} to="/about"><p className='px-4 py-2 '>ABOUT</p></NavLink>
                                <NavLink className='w-full' onClick={()=>setShowMenu(false)} to="/contact"><p className='px-4 py-2 '>CONTACT</p></NavLink>
                            </ul>
                        </div>
                    </div>
                </div>
